...
const Home = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      if (props && props.length > 0) setFeatured(props);
  ...
  return (
    <div className="bg-gray-50 min-h-screen w-full overflow-hidden font-sans">
         ...
         <div className="max-w-7xl mx-auto md:p-6 p-0">
            <div className="flex flex-col lg:flex-row shadow-2xl md:rounded-[4rem] rounded-none overflow-hidden bg-white min-h-[700px]">
               <div className="lg:w-[75%] relative h-[500px] lg:h-auto bg-black group overflow-hidden">
                  ...
                  {/* CONTROLES HERO <> */}
                  <button onClick={prevProp} className="absolute left-6 top-1/2 -translate-y-1/2 z-40 w-16 h-16 rounded-full border border-white/20 bg-black/20 text-white hover:bg-green-600 transition-all flex items-center justify-center backdrop-blur-md">
                     <FontAwesomeIcon icon={faChevronLeft} size="2x" />
                  </button>
                  <button onClick={nextProp} className="absolute right-6 top-1/2 -translate-y-1/2 z-40 w-16 h-16 rounded-full border border-white/20 bg-black/20 text-white hover:bg-green-600 transition-all flex items-center justify-center backdrop-blur-md">
                     <FontAwesomeIcon icon={faChevronRight} size="2x" />
                  </button>

                  <div className="absolute bottom-20 left-12 z-20 max-w-2xl text-left">
                     <span className="bg-green-600 text-white text-[10px] font-black px-4 py-2 rounded-full uppercase tracking-widest mb-6 inline-block shadow-lg">Selección de Élite</span>
                     ...
                     <button onClick={() => navigate(`/inmuebles/${current.id}`)} className="bg-green-600 text-white px-14 py-6 rounded-full font-black uppercase text-sm tracking-widest hover:bg-white hover:text-slate-900 transition-all shadow-3xl">Explorar Activo</button>
                  </div>

                  {/* MINIATURAS DEL MISMO INMUEBLE */}
                  <div className="absolute top-12 right-12 z-40 flex flex-col gap-4">
               ...
               <div className="lg:w-[25%] bg-slate-50 p-12 flex flex-col justify-center border-l border-slate-100 text-center">
                  <h3 className="text-slate-950 font-black text-3xl mb-10 tracking-tighter uppercase italic">Código <br/><span className="text-green-600 font-serif">A&C</span></h3>
                  <input className="w-full p-6 text-center border-2 border-slate-200 rounded-3xl bg-white text-xl font-black outline-none focus:border-green-500 transition-all mb-8 shadow-inner" placeholder="CÓDIGO A&C" value={quickCode} onChange={(e)=>setQuickCode(e.target.value)} />
      ...
      <section className="bg-slate-950 py-32 px-6 relative overflow-hidden">
         <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-24 relative z-10">
            <div className="lg:w-1/2 text-white text-left">
                <h2 className="text-5xl md:text-8xl font-black mb-10 leading-[0.8] tracking-tighter italic">¿Qué zona <br/><span className="text-green-600 underline font-serif">te interesa?</span></h2>
                <p className="text-slate-400 text-xl mb-12 max-w-lg">Nuestro equipo élite realizará el <strong>scouting exclusivo</strong> de mercado a tu medida.</p>
                <div className="bg-white p-3 rounded-full flex items-center shadow-3xl max-w-lg">
                   ...
                   <input className="bg-transparent border-none outline-none text-slate-950 placeholder-slate-400 flex-grow px-4 py-4 font-black" placeholder="Ej: Rosales, Chía..." value={zoneInput} onChange={(e) => setZoneInput(e.target.value)} />
                   <button onClick={() => setShowZoneModal(true)} className="bg-green-600 text-white w-16 h-16 rounded-full flex items-center justify-center hover:bg-green-700 transition-all"><FontAwesomeIcon icon={faPaperPlane} size="lg"/></button>
                </div>
            </div>
            <div className="lg:w-1/2 w-full grid grid-cols-1 md:grid-cols-2 gap-8">
               <div className="bg-white/5 border border-white/10 p-12 rounded-[4rem] group hover:bg-white/10 transition-all">
                  ...
                  <p className="text-xs text-slate-400 uppercase font-black tracking-widest italic">Días Promedio Venta</p>
               </div>
               <div className="bg-white/5 border border-white/10 p-12 rounded-[4rem] mt-12 group hover:bg-white/10 transition-all">
      ...
      <div className="py-32 px-6 max-w-7xl mx-auto">
         <h2 className="text-5xl font-black text-slate-900 tracking-tighter uppercase italic mb-20 text-left">Selección de Élite</h2>
      ...
      <section className="bg-white py-32 px-6 border-t border-slate-200">
         <div className="max-w-7xl mx-auto">
            <h2 className="text-center text-5xl font-black text-slate-950 mb-20 uppercase tracking-tighter italic">Visión <span className="text-green-600 underline font-serif">A&C</span></h2>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-12 text-left">
               {blogPosts.map(post => (
                  <Link to={`/blog/${post.id}`} key={post.id} className="group bg-white p-6 rounded-[3rem] shadow-xl hover:-translate-y-4 transition-all duration-500 border border-slate-50">
                     <div className="h-64 rounded-[2rem] overflow-hidden mb-10 shadow-2xl"><img src={post.image_url} className="w-full h-full object-cover grayscale group-hover:grayscale-0 transition-all" /></div>
                     <h3 className="font-black text-slate-950 text-2xl mb-4 leading-tight">{post.title}</h3>
                     <p className="text-slate-400 text-xs font-black uppercase tracking-widest italic">Leer Reporte <FontAwesomeIcon icon={faArrowRight} className="ml-2"/></p>
                  </Link>
               ))}
            </div>
         </div>
