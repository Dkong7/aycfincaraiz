...
const Home = () => {
  ...
  const handleZoneSearch = async () => {
    ...
    await supabase.from("scouting_requests").insert([{ zone_requested: zoneInput }]);
            ...
            <div className="flex flex-col lg:flex-row shadow-2xl md:rounded-[4rem] rounded-none overflow-hidden bg-white min-h-[700px]">
               <div className="lg:w-[75%] relative h-[500px] lg:h-auto bg-black group overflow-hidden">
                  ...
                  <div className="absolute bottom-20 left-12 z-20 max-w-2xl text-left">
                     ...
                     <button onClick={() => navigate(`/inmuebles/${current.id}`)} className="bg-green-600 text-white px-14 py-6 rounded-full font-black uppercase text-sm tracking-widest hover:bg-white hover:text-slate-900 transition-all shadow-3xl">Explorar Inmueble</button>
                  </div>
               ...
               <div className="lg:w-[25%] bg-slate-50 p-12 flex flex-col justify-center border-l border-slate-100 text-center">
                  ...
                  <input className="w-full p-6 text-center border-2 border-slate-200 rounded-3xl bg-white text-xl font-black outline-none focus:border-green-500 transition-all mb-8" placeholder="CÓDIGO A&C" value={quickCode} onChange={(e)=>setQuickCode(e.target.value)} />
      ...
      <div className="bg-slate-950 py-32 px-6 relative overflow-hidden">
         <div className="max-w-7xl mx-auto flex flex-col lg:flex-row items-center gap-24 relative z-10">
            <div className="lg:w-1/2 text-white text-left">
                ...
                <p className="text-slate-400 text-xl mb-12">Dinos dónde buscas y nuestro equipo realizará el **scouting exclusivo** de mercado por ti.</p>
                <div className="bg-white p-3 rounded-full flex items-center shadow-3xl max-w-lg">
                   ...
                   <input className="bg-transparent border-none outline-none text-slate-950 placeholder-slate-400 flex-grow px-4 py-4 font-black" placeholder="Ej: Rosales, Chía..." value={zoneInput} onChange={(e) => setZoneInput(e.target.value)} onKeyDown={(e) => e.key === "Enter" && handleZoneSearch()} />
            ...
            <div className="lg:w-1/2 w-full grid grid-cols-2 gap-8">
               <div className="bg-white/5 border border-white/10 p-12 rounded-[4rem] group hover:bg-white/10 transition-all">
                  ...
                  <p className="text-xs text-slate-400 uppercase font-black tracking-widest">Días Promedio Venta</p>
               </div>
               <div className="bg-white/5 border border-white/10 p-12 rounded-[4rem] mt-12 group hover:bg-white/10 transition-all">
                  ...
                  <p className="text-xs text-slate-400 uppercase font-black tracking-widest">Plusvalía Anual</p>
               </div>
            </div>
         </div>
      </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-12 text-left">
               {blogPosts.map(post => (
                  <Link to={`/blog/${post.id}`} key={post.id} className="group bg-white p-6 rounded-[3rem] shadow-xl hover:-translate-y-4 transition-all duration-500">
                     ...
                     <h3 className="font-black text-slate-950 text-2xl mb-4">{post.title}</h3>
