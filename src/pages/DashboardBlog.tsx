...
import { 
  ...
  Save, ArrowLeft, BookOpen, Loader2, X, CheckCircle 
} from "lucide-react";
import type { RecordModel } from "pocketbase";

// 1. IMPORTACIÓN CORREGIDA (Sin llaves en Quill)
import ReactQuill from 'react-quill-new';
import 'react-quill-new/dist/quill.snow.css';
import BlotFormatter from 'quill-blot-formatter';

// 2. ACCESO SEGURO A QUILL Y REGISTRO DE MÓDULO
// Usamos 'as any' para evitar el error de TypeScript ts(2614)
...
export default function DashboardBlog() {
  ...
  const openEditor = (post?: RecordModel) => {
      ...
      setPendingFormData(null); // Limpiar datos pendientes
  ...
  // --- MANEJADOR DE IMÁGENES (Subida a R2/Media) ---
  ...
  // --- PASO 1: PREPARAR PREVIEW (No guarda aún) ---
  const handlePreSave = (e: React.FormEvent) => {
    ...
    formData.set("content", contentBody); // Asegurar que el contenido del editor va en el form

    setPendingFormData(formData); // Guardamos los datos temporalmente
    ...
    <div className={`fixed inset-0 z-[100] flex font-sans transition-colors duration-500 ${s.mainBg}`}>
      ...
      <aside className={`w-64 flex flex-col py-6 border-r transition-colors duration-500 ${s.sidebar}`}>
         ...
         <nav className="px-3 space-y-1 flex-1">
            ...
            <button onClick={() => { setView("LIST"); setCurrentPost(null); }} className={`w-full text-left px-4 py-3 rounded-xl font-bold text-sm flex items-center gap-3 transition-all ${view === "LIST" ? s.activeBtn : `hover:opacity-80 ${s.sidebarText}`}`}>
      ...
      <main className="flex-1 p-8 overflow-y-auto">
         ...
         {view === "LIST" && (
            <div className={`rounded-xl shadow-lg border overflow-hidden ${s.card}`}>
               <div className="p-6 border-b border-gray-200/20 flex justify-between items-center">
                  <h2 className="font-black text-xl uppercase">Artículos</h2>
               ...
               <div className="overflow-x-auto">
                  <table className="w-full text-left">
                     <thead className="bg-black/5 text-[10px] font-bold uppercase">
                        <tr><th className="p-4">Artículo</th><th className="p-4">Autor</th><th className="p-4">Fecha</th><th className="p-4 text-right">Acciones</th></tr>
                     </thead>
         ...
         {/* EDITOR VISUAL */}
         {view === "EDITOR" && (
            <div className={`max-w-5xl mx-auto rounded-3xl shadow-xl border p-8 ${s.card}`}>
               <div className="flex justify-between items-center mb-8 border-b pb-4 border-gray-200/20">
                  <h2 className="text-2xl font-black uppercase flex items-center gap-2"><BookOpen size={24}/> {currentPost ? "Editar" : "Nuevo"} Post</h2>
                  <button onClick={() => setView("LIST")} className="text-xs font-bold uppercase opacity-50 hover:opacity-100">Cancelar</button>
               </div>

               {/* NOTA: Usamos onSubmit para desencadenar el PreSave */}
               <form onSubmit={handlePreSave} className="space-y-8">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                     <div className="md:col-span-2 space-y-6">
                         <div>
                            ...
                            <input name="title" defaultValue={currentPost?.title} required className={`w-full p-4 rounded-xl text-xl font-black outline-none border focus:ring-2 ring-current transition-all ${s.input}`} placeholder="Título del artículo..." />
                         </div>
                         <div>
                            <label className="block text-xs font-black uppercase tracking-widest mb-2 opacity-70">Resumen</label>
                            <textarea name="excerpt" defaultValue={currentPost?.excerpt} rows={3} className={`w-full p-4 rounded-xl outline-none border focus:ring-2 ring-current transition-all resize-none text-sm ${s.input}`} placeholder="Breve descripción..." />
                         </div>
                         <div>
                  ...
                  {/* BOTÓN PREVIEW */}
                  <button type="submit" disabled={loading || uploadingImg} className={`w-full ${s.btnPrimary} font-black uppercase tracking-widest py-4 rounded-xl shadow-lg transition-transform active:scale-95 flex items-center justify-center gap-2 text-lg`}>
                     {loading ? "Procesando..." : <><Save size={22}/> REVISAR Y PUBLICAR</>}
                  </button>
               </form>
            </div>
         )}
      ...
      {/* --- MODAL DE CONFIRMACIÓN Y PREVIEW --- */}
          ...
          <div className="fixed inset-0 z-[200] bg-black/80 backdrop-blur-sm flex items-center justify-center p-4 animate-in fade-in duration-200">
              <div className="bg-white rounded-3xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col relative">
                  ...
                  <div className="p-6 border-b flex justify-between items-center bg-gray-50">
                      <div>
                          <h3 className="text-xl font-black uppercase text-gray-800">Vista Previa</h3>
                          <p className="text-xs text-gray-500">Así se verá tu artículo publicado</p>
                      </div>
                      <button onClick={() => setShowConfirmModal(false)} className="p-2 hover:bg-gray-200 rounded-full transition-colors"><X size={20} className="text-gray-500"/></button>
                  </div>

                  {/* Body Modal (Scrollable) */}
                  <div className="flex-1 overflow-y-auto p-8 bg-gray-50">
                      <article className="max-w-3xl mx-auto bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
                          {/* Preview Hero */}
                          {heroPreview && (
                          ...
                          {/* Preview Excerpt */}
                          <div className="bg-gray-50 p-6 rounded-xl mb-8 border-l-4 border-blue-500 italic text-gray-700">
                              {pendingFormData.get("excerpt") as string}
                          </div>

                          {/* Preview Body */}
                          <div className="prose prose-lg max-w-none prose-img:rounded-xl prose-headings:font-bold prose-a:text-blue-600 ql-editor px-0"
                  ...
                  <div className="p-6 border-t bg-white flex justify-end gap-3 z-10">
                      <button 
                          ...
                          className="px-6 py-3 rounded-xl font-bold text-gray-500 hover:bg-gray-100 transition-colors uppercase text-sm"
                      >
                          Seguir Editando
                      </button>
                      <button 
                          ...
                          disabled={loading}
                          className="px-8 py-3 rounded-xl font-bold bg-green-600 hover:bg-green-700 text-white shadow-lg transition-transform active:scale-95 flex items-center gap-2 uppercase text-sm"
                      >
      ...
      <style>{`
         ...
         .ql-editor h2 { font-size: 1.5em; font-weight: 800; margin-top: 1em; margin-bottom: 0.5em; }
