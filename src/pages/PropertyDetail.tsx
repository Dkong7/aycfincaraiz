...
import { 
  ...
  TreePine, User, CheckCircle, X, ChevronLeft, ChevronRight, Play 
} from "lucide-react";
...
const PropertyDetail = () => {
  ...
  // 2. CONSUMIR CONTEXTO DE IDIOMA
  const context = useLanguage();
  const { translateDynamic } = context || { translateDynamic: (t: string) => t };

  // Función segura de traducción
  const safeTranslate = (text: string) => {
      if (typeof translateDynamic === 'function') return translateDynamic(text);
      return text;
  };
  ...
  if (loading) return <div className="min-h-screen bg-[#0A192F] flex items-center justify-center text-white font-bold tracking-widest">{safeTranslate("CARGANDO...")}</div>;
  if (!prop) return <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center"><h2 className="text-2xl font-bold mb-4 text-gray-800">{safeTranslate("Inmueble no encontrado")}</h2><Link to="/inmuebles" className="text-green-600 underline font-bold">{safeTranslate("Volver al listado")}</Link></div>;

  // --- LÓGICA DE MEDIA (VIDEO FIRST) ---
  ...
  const mediaList = [
      ...
      ...(prop.images?.map((img: string) => ({ 
          type: 'image', 
          src: `${PB_URL}/api/files/${prop.collectionId}/${prop.id}/${img}` 
      })) || [])
  ];

  if (mediaList.length === 0) {
      mediaList.push({ type: 'image', src: "https://via.placeholder.com/1200x800?text=SIN+FOTO" });
  }

  const nextImage = (e?: React.MouseEvent) => {
    e?.stopPropagation();
    setActiveImg((prev) => (prev + 1) % mediaList.length);
  };
  
  const prevImage = (e?: React.MouseEvent) => {
    e?.stopPropagation();
    setActiveImg((prev) => (prev - 1 + mediaList.length) % mediaList.length);
  };
  ...
  const getLabel = (key: string) => {
      ...
      // TRADUCIMOS LA ETIQUETA AL INSTANTE
      return safeTranslate(labels[key] || key.replace(/_/g, " "));
  };

  const getValue = (val: any) => {
      if (val === true) return safeTranslate("Sí");
      if (val === false) return safeTranslate("No");
      if (Array.isArray(val)) return val.length > 0 ? val.join(", ") : safeTranslate("Ninguna");
  ...
  return (
    <div className="bg-[#F3F4F6] min-h-screen font-sans">
      {/* 3. CORRECCIÓN ERROR NAVBAR: Se quitaron los props que causaban el error de IntrinsicAttributes */}
      <Navbar />
      ...
      >
         {/* LÓGICA DE RENDERIZADO HERO (Video Background o Imagen) */}
         {mediaList[activeImg].type === 'video' ? (
             <div className="w-full h-full pointer-events-none scale-125 relative">
                 <iframe 
                    className="w-full h-full opacity-80"
                    src={`https://www.youtube.com/embed/${mediaList[activeImg].src}?autoplay=1&mute=1&controls=0&loop=1&playlist=${mediaList[activeImg].src}&showinfo=0&rel=0&iv_load_policy=3&disablekb=1`}
                    title="Hero Video"
                    allow="autoplay; encrypted-media"
                 ></iframe>
             </div>
         ) : (
             <img src={mediaList[activeImg].src} className="w-full h-full object-cover transition-opacity duration-500 opacity-80" />
         )}
         ...
         <div className="absolute bottom-0 left-0 w-full p-6 md:p-16 max-w-7xl mx-auto pt-32 cursor-auto" onClick={(e) => e.stopPropagation()}>
            <Link to="/inmuebles" className="inline-flex items-center text-gray-300 hover:text-white mb-6 text-xs font-bold uppercase tracking-wider bg-black/30 px-4 py-2 rounded-full backdrop-blur-sm border border-white/10 transition-all hover:bg-black/50"><ArrowLeft size={14} className="mr-2"/> {safeTranslate("Volver al inventario")}</Link>
            ...
            <div className={`inline-flex items-center gap-2 px-3 py-1 rounded mb-4 text-xs font-black uppercase tracking-widest ${theme.color} text-white shadow-lg`}>
               {theme.icon} {safeTranslate(prop.property_type)}
            </div>
            
            <h1 className="text-3xl md:text-6xl font-black uppercase leading-tight mb-4 text-white drop-shadow-2xl">
               {safeTranslate(prop.title || "Inmueble Sin Título")}
            </h1>
            
            <div className="flex flex-col md:flex-row md:items-center gap-4 text-lg font-medium mb-6 text-gray-200">
               <span className="flex items-center gap-1">
                   <MapPin size={20} className="text-green-400"/> 
                   {/* Traducimos dirección/ubicación si es posible */}
                   {safeTranslate(prop.address_text || prop.address || prop.municipality)}, {safeTranslate(prop.neighborhood)}
               </span>
         ...
         <div className="lg:col-span-2 space-y-8">
            ...
            <div className="bg-white p-6 rounded-2xl shadow-xl border border-gray-100 grid grid-cols-2 md:grid-cols-4 gap-6 text-center">
               {[
                 ...
                 { k: 'antiquity', v: specs.antiquity || specs.building_age, unit: safeTranslate('años') },
            ...
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
               <h3 className="text-xl font-black uppercase text-[#0A192F] mb-6 flex items-center gap-2">
                   <FileText size={24} className="text-gray-400"/> {safeTranslate("Descripción")}
               </h3>
               <p className="text-gray-600 whitespace-pre-line leading-relaxed text-base">
                  {/* TRADUCCIÓN DE LA DESCRIPCIÓN */}
                  {safeTranslate(prop.description || "Sin descripción detallada disponible.")}
               </p>
            </div>

            {/* FICHA TÉCNICA DETALLADA */}
            <div className="bg-white p-8 rounded-2xl shadow-sm border border-gray-100">
               <h3 className="text-xl font-black uppercase text-[#0A192F] mb-8 flex items-center gap-2"><Layout size={24} className="text-gray-400"/> {safeTranslate("Detalles")}</h3>
               <div className="grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
                  ...
                  {specs.admin && (
                      <div className="flex justify-between items-center border-b border-gray-100 pb-2 px-2">
                          <span className="text-sm font-medium text-gray-500 flex items-center gap-2"><Receipt size={16} className={`${theme.text} opacity-70`}/> {safeTranslate("Administración")}</span>
            ...
            {/* GALERÍA GRID CON SOPORTE DE VIDEO */}
            {mediaList.length > 1 && (
               <div className="bg-white p-6 rounded-2xl shadow-sm border border-gray-100">
                   <h3 className="text-sm font-black uppercase text-gray-400 mb-4 tracking-widest">{safeTranslate("Galería Completa")}</h3>
         ...
         <div className="lg:col-span-1">
             <div className="bg-white p-8 rounded-2xl shadow-xl border border-gray-100 sticky top-24">
                <div className="text-center mb-8">
                    ...
                    <p className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1">{safeTranslate("Agente Responsable")}</p>
                ...
                <div className="space-y-3">
                    <a href={`https://wa.me/573134663832?text=Hola, estoy interesado en el inmueble ${prop.ayc_id} (${prop.title})`} target="_blank" 
                       ...
                       {safeTranslate("Contactar por WhatsApp")}
                    </a>
                    ...
                    <button className="block w-full bg-[#0A192F] hover:bg-[#112] text-white font-bold py-4 rounded-xl text-center transition-all shadow-lg flex items-center justify-center gap-2">
                       <Calendar size={18}/> {safeTranslate("Agendar Visita")}
                    </button>
                </div>
                ...
                <div className="mt-8 pt-6 border-t border-gray-100">
                   <div className="flex items-start gap-3 p-4 bg-green-50 rounded-xl">
                       ...
                       <div>
                           <h4 className="font-bold text-sm text-green-800">{safeTranslate("¿Necesitas Crédito?")}</h4>
                           <p className="text-xs text-green-700 mt-1 leading-relaxed">{safeTranslate("Tenemos aliados financieros para ayudarte a conseguir este inmueble.")}</p>
                           <Link to="/servicios" className="text-xs font-black text-green-800 underline mt-2 block hover:text-green-900">{safeTranslate("Ver Aliados")}</Link>
                       </div>
                   </div>
                </div>
             </div>
