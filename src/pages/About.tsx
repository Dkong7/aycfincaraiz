      ...
      <div className="relative bg-[#0A192F] text-white py-24 text-center overflow-hidden pt-32">
        ...
        <div className="relative z-10 container mx-auto px-6 animate-fade-in-up">
          ...
          <p className="text-xl md:text-2xl text-blue-100 font-light max-w-3xl mx-auto leading-relaxed">
             {t('about_hero') || "Más que una inmobiliaria, somos tus socios estratégicos en la construcción de patrimonio."}
          </p>
        </div>
      ...
      <div className="max-w-7xl mx-auto px-6 sm:px-8 lg:px-12 mt-16 lg:mt-24 space-y-24 pb-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-16 items-center">
           <div className="order-2 lg:order-1 animate-fade-in-right">
              <div className="flex items-center gap-4 mb-6">
                 ...
                 <span className="font-bold uppercase tracking-widest text-[#0A192F]">{t('about_story_title') || "NUESTRA HISTORIA"}</span>
              </div>
              <h2 className="text-3xl md:text-5xl font-bold text-gray-900 mb-8 leading-tight">
                 {t('about_story_headline') || "Excelencia inmobiliaria desde 2010"}
              </h2>
              <p className="text-lg text-gray-600 leading-relaxed mb-8 text-justify">
                {t('about_story_desc') || "Fundada con la visión de transformar el mercado inmobiliario en Bogotá, AyC ha crecido basándose en la confianza, la transparencia y resultados comprobables."}
              </p>
              <div className="grid grid-cols-3 gap-8 text-center border-t border-gray-100 pt-8">
                 <div>
                    ...
                    <p className="text-sm text-gray-500 uppercase font-bold">{t('about_stats_clients') || "Clientes Felices"}</p>
                 </div>
                 <div>
                    <p className="text-4xl font-bold text-[#0A192F] mb-2">15</p>
                    <p className="text-sm text-gray-500 uppercase font-bold">{t('about_stats_exp') || "Años Exp."}</p>
                 </div>
                 <div>
                    <p className="text-4xl font-bold text-[#0A192F] mb-2">$80MM+</p>
                    <p className="text-sm text-gray-500 uppercase font-bold">{t('about_stats_sales') || "En Ventas"}</p>
                 </div>
              </div>
           </div>
        ...
        <div className="bg-slate-50 p-12 rounded-[3rem]">
            <div className="text-center mb-12">
               <h2 className="text-3xl font-bold text-[#0A192F]">{t('about_values_title') || "Nuestros Valores"}</h2>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
               <div className="bg-white p-8 rounded-2xl shadow-sm hover:shadow-lg transition-all hover:-translate-y-1 text-center">
                  ...
                  <h3 className="font-bold text-xl mb-2 text-[#0A192F]">{t('val1_title') || "Integridad"}</h3>
                  <p className="text-gray-500 text-sm">{t('val1_desc') || "Actuamos con honestidad y transparencia en cada negocio."}</p>
               </div>
               <div className="bg-white p-8 rounded-2xl shadow-sm hover:shadow-lg transition-all hover:-translate-y-1 text-center">
                  <FontAwesomeIcon icon={faChartLine} className="text-4xl text-blue-500 mb-4" />
                  <h3 className="font-bold text-xl mb-2 text-[#0A192F]">{t('val2_title') || "Excelencia"}</h3>
                  <p className="text-gray-500 text-sm">{t('val2_desc') || "Buscamos superar las expectativas con resultados superiores."}</p>
               </div>
               <div className="bg-white p-8 rounded-2xl shadow-sm hover:shadow-lg transition-all hover:-translate-y-1 text-center">
                  <FontAwesomeIcon icon={faHeart} className="text-4xl text-pink-500 mb-4" />
                  <h3 className="font-bold text-xl mb-2 text-[#0A192F]">{t('val3_title') || "Pasión"}</h3>
                  <p className="text-gray-500 text-sm">{t('val3_desc') || "Amamos lo que hacemos y se nota en cada detalle."}</p>
               </div>
            </div>
        </div>
        ...
        <div>
           <div className="text-center mb-16 animate-fade-in-up">
              <h2 className="text-3xl md:text-5xl font-bold text-gray-900 mb-4">{t('about_team_title') || "Liderazgo"}</h2>
           ...
           <div className="grid grid-cols-1 md:grid-cols-2 gap-12 max-w-5xl mx-auto">
             ...
             <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-gray-100 text-center hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 group relative overflow-hidden">
                ...
                <p className="text-pink-600 font-bold text-xs uppercase tracking-widest mb-6">{t('role_founder1') || "Cofundadora & Gerente Comercial"}</p>
                <p className="text-gray-600 leading-relaxed mb-8">
                  {t('desc_founder1') || "Experta en negociación y relaciones públicas con más de 15 años liderando equipos comerciales de alto rendimiento."}
                </p>
             ...
             <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-gray-100 text-center hover:shadow-2xl hover:-translate-y-2 transition-all duration-300 group relative overflow-hidden">
                ...
                <p className="text-blue-900 font-bold text-xs uppercase tracking-widest mb-6">{t('role_founder2') || "Cofundador & Director Jurídico"}</p>
                <p className="text-gray-600 leading-relaxed mb-8">
                  {t('desc_founder2') || "Abogado especialista en derecho inmobiliario y urbano. Garante de la seguridad jurídica en cada operación de la firma."}
                </p>
                <SocialLinks linkedin="https://linkedin.com" instagram="https://instagram.com" />
             </div>
           </div>
