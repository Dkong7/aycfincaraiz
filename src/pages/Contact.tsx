       ...
       <div className="pt-40 pb-24 max-w-7xl mx-auto px-6 grid grid-cols-1 lg:grid-cols-2 gap-20 items-center relative z-10">
          ...
          <div className="animate-fade-in-up">
              ...
              <p className="text-slate-600 text-lg md:text-xl mb-12 leading-relaxed border-l-4 border-green-500 pl-6 max-w-lg font-medium">
                 {t('contact_hero_desc') || "¿Buscas comprar, vender o un avalúo certificado? Estamos listos para asesorarte con total confidencialidad."}
              </p>
              ...
              <div className="space-y-10 mb-16">
                 ...
                 <div className="flex items-center gap-6 group">
                    ...
                    <div>
                       <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">{t('contact_call') || "Llámanos"}</p>
                 ...
                 <div className="flex items-center gap-6 group">
                    ...
                    <div>
                       <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mb-1">{t('contact_write') || "Escríbenos"}</p>
          ...
          <div className="bg-white p-8 md:p-12 rounded-[2.5rem] shadow-2xl border border-slate-200 relative overflow-hidden animate-fade-in-up delay-200">
              ...
              <form onSubmit={handleSubmit} className="space-y-6 relative z-10">
                 <div className="flex justify-between items-center mb-8">
                     <h3 className="text-3xl font-black text-[#0A192F]">{t('form_title') || "Escríbenos"}</h3>
                     {Object.keys(errors).length > 0 && <AlertTriangle size={24} className="text-red-500 animate-bounce"/>}
                 </div>

                 <div>
                    <label className="block text-xs font-extrabold text-slate-400 mb-2 uppercase tracking-wide ml-1">{t('form_name') || "Nombre"}</label>
                    <input 
                      type="text" 
                      name="name"
                      value={formData.name}
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    <div>
                       <label className="block text-xs font-extrabold text-slate-400 mb-2 uppercase tracking-wide ml-1">{t('form_phone') || "Teléfono"}</label>
                       <input 
                         type="text" 
                         name="phone"
                    ...
                    <div>
                       <label className="block text-xs font-extrabold text-slate-400 mb-2 uppercase tracking-wide ml-1">{t('form_email') || "Email"}</label>
                       <input 
                         type="email" 
                         name="email"
                 ...
                 <div>
                    <label className="block text-xs font-extrabold text-slate-400 mb-2 uppercase tracking-wide ml-1">{t('form_msg') || "Mensaje"}</label>
                    <textarea 
                      rows={4} 
                      name="msg"
                      value={formData.msg}
                 ...
                 >
                    {isSubmitting ? (
                        <>{t('form_sending') || "Enviando Seguro"} <div className="w-4 h-4 border-2 border-white border-t-transparent rounded-full animate-spin"></div></>
                    ) : (
                        t('form_btn') || "ENVIAR MENSAJE"
                    )}
                 </button>
                 ...
                 <p className="text-center text-[10px] text-gray-400 font-bold uppercase tracking-wider mt-6 flex items-center justify-center gap-1.5">
                     <Lock size={10} /> {t('security_note') || "Protegido por AyC Shield v1.0"}
                 </p>
              </form>
          </div>
