...
export default function DashboardInventario() {
  ...
  const showModal = (config: ModalConfig) => {
      setModalState({ isOpen: true, config: { ...config, theme: modalTheme } });
  };

  const heroItems = properties.filter(p => p.is_hero).sort((a, b) => new Date(a.updated).getTime() - new Date(b.updated).getTime());
  const getHeroIndex = (id: string) => { const index = heroItems.findIndex(h => h.id === id); return index !== -1 ? index + 1 : null; };
  const counts = { hero: heroItems.length, reina: properties.filter(p => p.is_opportunity).length, fav: properties.filter(p => p.is_ayc_favorite).length };
  const remaining = { hero: Math.max(0, 5 - counts.hero), reina: Math.max(0, 1 - counts.reina), fav: Math.max(0, 10 - counts.fav) };
  ...
  const handleEdit = async (propId: string) => {
      ...
      try {
          ...
          showModal({ type: 'error', title: "Error", msg: "No se pudo cargar el inmueble." });
      } finally {
  ...
  const confirmDelete = (id: string) => {
      showModal({ type: 'confirm', title: "¿Eliminar?", msg: "El borrador desaparecerá permanentemente.", onConfirm: () => handleDelete(id) });
  };
  ...
  const handleDelete = async (id: string) => {
      try {
          ...
          if(editingProp?.id === id) { setEditingProp(null); setView("INVENTORY"); }
          showModal({ type: 'success', title: "Eliminado", msg: "Registro borrado." });
      } catch(e:any) { showModal({ type: 'error', title: "Error", msg: "No se pudo eliminar." }); }
  };
  ...
  return (
    <div className={`flex flex-col md:flex-row font-sans transition-colors duration-500 ${s.mainBg} min-h-screen md:h-screen md:overflow-hidden`}>
      ...
      <header className={`md:hidden flex justify-between items-center p-4 border-b transition-colors duration-500 bg-white/95 backdrop-blur shadow-sm sticky top-0 z-50`}>
          <div className="flex items-center gap-2"><div className="w-8 h-8 bg-green-600 rounded flex items-center justify-center text-white font-black text-xs">AYC</div><h1 className="font-black text-lg tracking-tighter text-[#0A192F]">DASHBOARD</h1></div>
          <button onClick={() => setIsMobileMenuOpen(true)} className="text-[#0A192F] p-2 hover:bg-gray-100 rounded-lg transition-colors"><Menu size={24} /></button>
      </header>
      ...
      <main className="flex-1 p-4 md:p-8 w-full relative z-0 md:overflow-y-auto">
         {view === "INVENTORY" ? (
                         ...
                         <tbody className="divide-y divide-gray-100 text-gray-600">
                            {properties.map((p) => {
                               ...
                               <tr key={p.id} className={`hover:bg-gray-50 transition-colors ${isDraft ? "bg-yellow-50/60 opacity-60 grayscale-[20%]" : ""}`}>
                                  ...
                                  <td className="p-4 text-center font-mono text-xs font-bold text-green-600">{p.price_cop ? formatCurrency(p.price_cop) : <span className="text-gray-400">--</span>}</td>
                                  <td className="p-4 text-center"><button onClick={() => toggle(p.id, "is_hero", p.is_hero)} disabled={isDraft} className={`relative flex items-center justify-center mx-auto w-10 h-10 rounded-full transition-all ${isDraft ? "opacity-20 cursor-not-allowed" : p.is_hero ? "bg-blue-100 ring-2 ring-blue-300 scale-110 shadow-sm" : "hover:bg-gray-100 text-gray-300"}`}>{p.is_hero ? <><Zap size={24} className="text-blue-300 absolute" fill="currentColor"/><span className="relative z-10 text-sm font-black text-blue-900">{heroOrder}</span></> : <Zap size={20}/>}</button></td>
                                  <td className="p-4 text-center"><button onClick={() => toggle(p.id, "is_opportunity", p.is_opportunity)} disabled={isDraft} className={`p-1.5 rounded-full transition-all ${isDraft ? "opacity-20 cursor-not-allowed" : p.is_opportunity ? "bg-yellow-100 text-yellow-500 ring-2 ring-yellow-200 shadow-lg scale-110" : "text-gray-300 hover:bg-gray-100"}`}><Crown size={16} fill={p.is_opportunity ? "currentColor" : "none"}/></button></td>
                                  <td className="p-4 text-center"><button onClick={() => toggle(p.id, "is_ayc_favorite", p.is_ayc_favorite)} disabled={isDraft} className={`p-1.5 rounded-full transition-all ${isDraft ? "opacity-20 cursor-not-allowed" : p.is_ayc_favorite ? "bg-green-100 text-green-500 ring-2 ring-green-200 scale-110" : "text-gray-300 hover:bg-gray-100"}`}><Star size={16} fill={p.is_ayc_favorite ? "currentColor" : "none"}/></button></td>
                                  <td className="p-4 text-right"><div className="flex items-center justify-end gap-2"><Link to={`/inmuebles/${p.ayc_id || p.id}`} target="_blank" className={`p-2 rounded transition-colors ${isDraft ? "text-gray-300 cursor-not-allowed" : "text-gray-500 hover:bg-gray-100 hover:text-black"}`} title="Ver en Web" onClick={(e) => isDraft && e.preventDefault()}><Eye size={16}/></Link><button onClick={() => handleEdit(p.id)} className="p-2 text-blue-500 hover:bg-blue-50 rounded transition-colors" title="Editar"><Edit size={16}/></button><button onClick={() => confirmDelete(p.id)} className="p-2 text-red-500 hover:bg-red-50 rounded transition-colors" title="Borrar"><Trash size={16}/></button></div></td>
                               </tr>
                               );
                            })}
                         </tbody>
            ...
            <CreatePropertyForm key={formKey} initialData={editingProp} onSuccess={handleSuccessSave} />
         )}
      </main>
    </div>
  );
