...
import { 
  Maximize, ArrowUpFromLine, Car, Bath, CheckCircle2, Briefcase, 
  Building2, DollarSign, TrendingUp, RefreshCw, Users, Server
} from "lucide-react";
import { useApp } from "../../context/AppContext"; 
import { useTRM } from "../../hooks/useTRM";       
import { translate as localTranslate, OFICINA_ICONS } from "./oficina.config";
...
export default function OficinaDetailView({ specs, description, adminFee, priceCop, priceUsd }: any) {
  ...
  // --- SUB-COMPONENTES UI (Tema Esmeralda) ---
    ...
    <div className="space-y-12 animate-in slide-in-from-bottom-4 duration-700 font-sans">
       ...
       <section>
          <div className="flex items-center gap-2 mb-6">
             ...
             <h3 className="font-black text-sm text-gray-400 uppercase tracking-widest">{translateDynamic("Ficha Corporativa")}</h3>
          </div>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
             <MainStat label="Área Privada" val={`${specs.area || 0} m²`} icon={Maximize} />
             <MainStat label="Ubicación" val={`Piso ${specs.floor_num || "--"}`} icon={ArrowUpFromLine} />
             <MainStat label="Garajes" val={specs.parking} icon={Car} sub="Privados" />
             <MainStat label="Tipo Edificio" val={specs.building_type} icon={Building2} />
          </div>
       </section>
          ...
          <div className="lg:col-span-2 space-y-8">
             <div className="bg-white p-8 rounded-[2rem] border border-gray-100 shadow-sm relative overflow-hidden">
                ...
                <h3 className="font-black text-xl text-gray-800 mb-4 flex items-center gap-2 relative z-10">
                   <Briefcase size={20} className="text-emerald-600"/> {translateDynamic("Descripción de la Oficina")}
                </h3>
          ...
          <div className="space-y-6">
             <div className="bg-white p-6 rounded-3xl border border-gray-200 shadow-sm">
                <h3 className="font-black text-sm text-gray-800 uppercase mb-4 border-b pb-2 flex items-center gap-2">
                    <CheckCircle2 size={16} className="text-emerald-600"/> {translateDynamic("Detalles & Costos")}
                </h3>
                <div className="flex flex-col">
                   ...
                   <SpecRow label="Estrato" val={specs.stratum} icon={ArrowUpFromLine} />
       ...
       {/* SECCIÓN 3: INFRAESTRUCTURA & AMENIDADES */}
       {specs.features && specs.features.length > 0 && (
         <section>
            <h3 className="font-black text-sm text-emerald-600 uppercase tracking-widest mb-5 flex items-center gap-2 border-b border-emerald-100 pb-2">
               <Server size={16}/> {translateDynamic("Infraestructura & Dotación")}
            </h3>
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
               {specs.features.map((feat: string) => {
                  const Icon = OFICINA_ICONS[feat] || CheckCircle2;
                  return (
                    <div key={feat} className="flex items-center gap-2 px-4 py-3 rounded-xl bg-white border border-gray-100 text-sm font-bold text-gray-600 shadow-sm hover:shadow-md hover:border-emerald-200 transition-all cursor-default border-l-4 border-l-emerald-500">
