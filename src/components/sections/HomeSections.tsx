...
import { 
  ArrowRight, Crown, Star, CheckCircle2,
                ...
                <div className="w-full lg:w-[30%] flex flex-col justify-center space-y-6 z-20 order-1 lg:order-1">
                    <div className="animate-in fade-in slide-in-from-bottom-5 duration-700">
                        ...
                        <p className="text-gray-500 mt-3 flex items-center gap-2 text-sm uppercase font-bold tracking-wide">
                            <MapPin size={16} className="text-yellow-500"/>{opportunity.neighborhood || opportunity.municipality}
                        </p>
                    </div>
                        ...
                        <div className="absolute bottom-8 left-8 right-8 hidden md:block">
                            <div className="bg-black/70 backdrop-blur-xl rounded-2xl p-5 flex justify-around items-center border border-white/10 text-white shadow-2xl">
                                {(() => { const s = getSpecs(opportunity); return ( <> 
                                    ...
                                    {(s.area_built || s.area_total) && <div className="w-px h-8 bg-white/20"></div>}
                                    ...
                                    {(s.habs || s.rooms) && <div className="w-px h-8 bg-white/20"></div>}
                                    ...
                                    {(s.baths || s.bathrooms) && <div className="w-px h-8 bg-white/20"></div>}
                            ...
                            <Link key={prop.id} to={`/inmuebles/${prop.id}`} className="group bg-white rounded-2xl overflow-hidden shadow-lg hover:shadow-2xl transition-all duration-300 border border-gray-100 flex flex-col h-full hover:-translate-y-2">
                                <div className="h-60 overflow-hidden relative">
                                    ...
                                    {prop.images?.[0] ? (
                                        ...
                                        <div className="w-full h-full bg-gray-200 flex items-center justify-center">N/A</div>
                                    )}
                                    <div className="absolute top-4 left-4 z-20"><span className="bg-black/60 backdrop-blur-sm text-white text-[10px] font-bold px-3 py-1 rounded-full uppercase border border-white/20 shadow-sm">{prop.property_type}</span></div>
                                </div>
                                <div className="p-6 flex flex-col flex-1">
                                    <h4 className="font-bold text-gray-800 text-lg leading-tight mb-2 group-hover:text-green-600 transition-colors line-clamp-2">{prop.title}</h4>
                                    <div className="flex items-center gap-2 text-xs text-gray-400 uppercase font-bold mb-6"><MapPin size={14} className="text-green-500"/> <span className="truncate">{prop.neighborhood || prop.municipality}</span></div>
            ...
            <div className="flex flex-col lg:flex-row items-center gap-16">
                <div className="lg:w-1/2 space-y-8">
                    <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-green-100 text-green-700 text-xs font-black uppercase tracking-widest">
                        <BadgeCheck size={16} /> Certificados
                    </div>
                    ...
                    <Link to="/servicios/avaluos" className="inline-flex items-center justify-center h-14 px-8 rounded-full bg-slate-900 text-white font-black uppercase tracking-wide hover:bg-green-600 hover:shadow-lg transition-all transform hover:-translate-y-1">SOLICITAR AVALÚO</Link>
                </div>
                <div className="lg:w-1/2 relative group">
                    <div className="relative h-[500px] w-full bg-white rounded-[3rem] shadow-2xl overflow-hidden border-8 border-white">
                        ...
                        <div className="absolute bottom-10 left-10 bg-white/90 backdrop-blur-md p-6 rounded-2xl shadow-xl border border-white/20 animate-bounce-slow">
                            <div className="flex items-center gap-4"><div className="p-3 bg-green-100 rounded-full text-green-600"><TrendingUp size={24}/></div><div><p className="text-xs text-slate-400 font-bold uppercase">Valorización</p><p className="text-xl font-black text-slate-800">+12.5% Anual</p></div></div>
                        </div>
                        <div className="absolute top-10 right-10 bg-slate-900/90 backdrop-blur-md p-4 rounded-xl shadow-xl border border-white/10 text-white">
                            <div className="flex items-center gap-3"><FileText className="text-green-400" size={20}/><div className="text-xs"><p className="font-bold uppercase tracking-wider opacity-70">Certificación</p><p className="font-black text-sm">RAA / Lonja</p></div></div>
                        </div>
                    </div>
                </div>
            </div>
      ...
      <section className="py-24 bg-white relative overflow-hidden border-t border-slate-100">
        <div className="max-w-7xl mx-auto px-6">
            <div className="text-center max-w-3xl mx-auto mb-16">
                <div className="inline-flex items-center gap-2 mb-4 text-blue-600 font-black uppercase tracking-widest text-xs bg-blue-50 px-4 py-2 rounded-full"><Shield size={14} /> Legalidad Garantizada</div>
                <h2 className="text-4xl md:text-5xl font-black text-slate-900 mb-6 uppercase leading-none">Seguridad Jurídica <br/><span className="text-slate-300">Inmobiliaria</span></h2>
                <p className="text-lg text-slate-500">Protegemos su patrimonio con estudios de títulos rigurosos y saneamiento predial.</p>
            </div>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
                <div className="group relative bg-slate-50 hover:bg-slate-900 rounded-[2.5rem] p-10 transition-all duration-500 overflow-hidden">
                    ...
                    <div className="relative z-10 h-full flex flex-col justify-between">
                        ...
                        <div><h3 className="text-2xl font-black text-slate-900 group-hover:text-white mb-2">Estudio de Títulos</h3><p className="text-slate-500 group-hover:text-slate-300 text-sm leading-relaxed">Análisis profundo de la tradición del inmueble para garantizar una compra segura.</p></div>
                    </div>
                </div>
                <div className="group relative bg-slate-900 rounded-[2.5rem] p-10 transform md:-translate-y-4 shadow-2xl shadow-slate-200">
                    ...
                    <div className="relative z-10 h-full flex flex-col justify-between">
                        ...
                        <div><h3 className="text-2xl font-black text-white mb-2">Saneamiento Predial</h3><p className="text-slate-300 text-sm leading-relaxed">Resolvemos embargos, sucesiones y limitaciones al dominio.</p></div>
                        <Link to="/servicios/juridico" className="mt-8 text-center py-3 rounded-xl bg-white text-slate-900 font-bold uppercase text-xs tracking-wider hover:bg-blue-50 transition-colors">Ver Servicio</Link>
                    </div>
                </div>
                <div className="group relative bg-slate-50 hover:bg-slate-900 rounded-[2.5rem] p-10 transition-all duration-500 overflow-hidden">
                    ...
                    <div className="relative z-10 h-full flex flex-col justify-between">
                        ...
                        <div><h3 className="text-2xl font-black text-slate-900 group-hover:text-white mb-2">Contratos Seguros</h3><p className="text-slate-500 group-hover:text-slate-300 text-sm leading-relaxed">Elaboración de promesas de compraventa y contratos blindados.</p></div>
                    </div>
                </div>
            </div>
        </div>
      ...
      <section className="bg-slate-950 py-32 px-6 relative overflow-hidden text-left border-t border-white/5">
        <div className="max-w-7xl mx-auto flex flex-col lg:flex-row-reverse items-center gap-24 relative z-10">
          <div className="lg:w-1/2">
            <span className="text-green-500 font-black uppercase tracking-[0.4em] text-[10px] mb-8 block">Producción de Lujo</span>
            <h2 className="text-5xl md:text-8xl font-black text-white leading-[0.85] mb-10 tracking-tighter italic">Marketing <br/><span className="text-transparent bg-clip-text bg-gradient-to-r from-green-400 to-blue-400 underline font-serif">Audiovisual</span></h2>
            <p className="text-xl text-slate-400 mb-12 max-w-md font-light leading-relaxed">Elevamos el nivel de su propiedad con tomas aéreas 4K, recorridos inmersivos y fotografía profesional.</p>
            <div className="grid grid-cols-2 gap-10 mb-14 font-sans text-white">
              <div className="flex items-center gap-4 group cursor-default"><FontAwesomeIcon icon={faPlane} className="text-green-500 text-3xl group-hover:scale-110 transition-all" /><span className="font-black uppercase text-[10px] tracking-widest border-b border-green-900 pb-2">Drones 4K</span></div>
              <div className="flex items-center gap-4 group cursor-default"><FontAwesomeIcon icon={faVrCardboard} className="text-blue-400 text-3xl group-hover:scale-110 transition-all" /><span className="font-black uppercase text-[10px] tracking-widest border-b border-blue-900 pb-2">Recorridos 360°</span></div>
            </div>
            <Link to="/servicios/audiovisual" className="inline-block bg-white text-slate-950 font-black px-14 py-6 rounded-full hover:bg-green-600 hover:text-white transition-all shadow-3xl uppercase text-xs tracking-widest">Ver Portafolio <FontAwesomeIcon icon={faArrowRightSolid} className="ml-2" /></Link>
          </div>
          <div className="lg:w-1/2 relative group">
