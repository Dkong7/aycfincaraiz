...
const Footer = () => {
  ...
  const testimonials = [
    { id: 1, quote: "Gracias al avalúo certificado de A&C logramos cerrar la venta...", name: "Carlos M.", role: "Logística S.A.S", img: "https://images.unsplash.com/photo-1566492031773-4f4e44671857?auto=format&fit=crop&q=80&w=100&h=100" },
    { id: 2, quote: "La asesoría jurídica nos salvó de un negocio con problemas...", name: "Dra. Elena R.", role: "Inversionista", img: "https://images.unsplash.com/photo-1573496359142-b8d87734a5a2?auto=format&fit=crop&q=80&w=100&h=100" },
    { id: 3, quote: "El video con dron vendió mi casa campestre en Chía...", name: "Familia Torres", role: "Venta Residencial", img: "https://images.unsplash.com/photo-1542596594-649edbc13630?auto=format&fit=crop&q=80&w=100&h=100" },
    { id: 4, quote: "Excelente alianza para la comercialización de nuestro proyecto VIS...", name: "Ing. Roberto P.", role: "Constructora Arcos", img: "https://images.unsplash.com/photo-1507003211169-0a1dd7228f2d?auto=format&fit=crop&q=80&w=100&h=100" },
    ...
    { id: 6, quote: "La gestión de saneamiento predial fue rápida y transparente...", name: "Don Jorge V.", role: "Propietario", img: "https://images.unsplash.com/photo-1552058544-f2b08422138a?auto=format&fit=crop&q=80&w=100&h=100" },
    { id: 7, quote: "Su análisis de mercado nos permitió ajustar los cánones...", name: "Marta G.", role: "Admin. Propiedad Horizontal", img: "https://images.unsplash.com/photo-1580489944761-15a19d654956?auto=format&fit=crop&q=80&w=100&h=100" },
    { id: 8, quote: "Atención personalizada y muy humana...", name: "Andrés y Paula", role: "Primer Hogar", img: "https://images.unsplash.com/photo-1517486808906-6ca8b3f04846?auto=format&fit=crop&q=80&w=100&h=100" },
    { id: 9, quote: "Como inversionista extranjero, necesitaba confianza local...", name: "David C.", role: "Inversión LatAm", img: "https://images.unsplash.com/photo-1472099645785-5658abf4ff4e?auto=format&fit=crop&q=80&w=100&h=100" },
    { id: 10, quote: "Profesionalismo puro. Los reportes de avalúo bajo norma NIIF...", name: "Clara B.", role: "Directora Financiera", img: "https://images.unsplash.com/photo-1573497019940-1c28c88b4f3e?auto=format&fit=crop&q=80&w=100&h=100" }
  ];
           ...
           <div className="bg-slate-700/50 p-5 rounded-xl border border-slate-600 relative min-h-[150px] flex flex-col justify-between transition-all duration-500 group">
              ...
              <div key={activeTestimonial.id} className="animate-fade-in relative z-10 px-2">
                  ...
                  <div className="flex items-center gap-3 border-t border-slate-600 pt-3">
                     ...
                     <div><p className="font-bold text-white text-xs">{activeTestimonial.name}</p><p className="text-[10px] text-yellow-500 uppercase font-bold">{activeTestimonial.role}</p></div>
                  </div>
              </div>
              <div className="absolute top-1/2 -translate-y-1/2 left-0 right-0 flex justify-between px-1 opacity-0 group-hover:opacity-100 transition-opacity duration-300 pointer-events-none z-20">
                 <button onClick={prevTestimonial} className="pointer-events-auto w-7 h-7 bg-slate-800/80 hover:bg-yellow-500 text-white hover:text-slate-900 rounded-full flex items-center justify-center transition text-xs backdrop-blur-sm border border-slate-600 shadow-lg"><FontAwesomeIcon icon={faChevronLeft} /></button>
                 <button onClick={nextTestimonial} className="pointer-events-auto w-7 h-7 bg-slate-800/80 hover:bg-yellow-500 text-white hover:text-slate-900 rounded-full flex items-center justify-center transition text-xs backdrop-blur-sm border border-slate-600 shadow-lg"><FontAwesomeIcon icon={faChevronRight} /></button>
              </div>
              <div className="flex justify-center gap-1 mt-3 z-10 relative">
                 {testimonials.map((_, idx) => (<div key={idx} className={`h-1 rounded-full transition-all duration-300 ${idx === currentIdx ? "w-3 bg-yellow-500" : "w-1 bg-slate-600"}`}></div>))}
              </div>
           </div>
        </div>
      </div>
